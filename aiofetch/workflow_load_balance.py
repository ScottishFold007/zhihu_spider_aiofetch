...
def load_balance(fetch_body: list, func: callable, work_flow_num: int,
    ...
    flood_discharge = max(min_flood_discharge, int(
        min(1., flood_discharge_ratio) * floodplain_size))
    ...
    while fetch_body:
        ...
        if (cur_task['task_count'] > 100) and (cur_task['task_count'] +
                                               work_flow_task_count[min_work_flow_index] > work_flow_capacity +
                                               floodplain_size):
